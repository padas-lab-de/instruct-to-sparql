...
def add_instructions_to_data(split="all", dataset="final_fq18_without_limit"):
    """Add processed instructions to the data"""
    ...
    generated_instructions_df = pd.DataFrame(generated_instructions, columns=["id", "instructions"]).astype(
        {"id": int}).set_index("id")
    generated_instructions_df.drop(rows_to_drop, inplace=True)
    generated_instructions_df.reset_index(drop=True, inplace=True)
    generated_instructions_df.columns = ["instructions"]
    ...
    if split == "full":
        ...
        data.columns = ["sparql_raw", "sparql_annotated", "query_results", "sparql_query", "instructions", "complexity",
                        "complexity_description"]
        data["id"] = data.index
        data = data[["id", "instructions", "sparql_raw", "sparql_annotated", "sparql_query", "complexity",
